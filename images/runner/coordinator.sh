...
ensure_state_fields_initialized() {
  ...
  # v06MilestoneStatus (issue #1789, #1799): tracks whether v0.6 Collective Action milestone is complete.
  ...
  # v06CriteriaStatus (issue #1789, #1799): human-readable status of last v0.6 criteria check.
...
# ── v0.6 Collective Action Milestone Checker (issue #1789, fixed by #1799) ───
#
...
#
# Issue #1799: Reads from s3://agentex-thoughts/swarm-memories/*.json
# (consistent with write_swarm_memory() in helpers.sh which writes to swarm-memories/).
# The original PR #1794 had a path mismatch: it read from "swarms/" instead of "swarm-memories/".
#
# Issue #1799: Criterion 3 checks .goalOrigin == "agent-proposed" or "emergent".
# write_swarm_memory() now includes goalOrigin field (fixed in this PR).
#
...
check_v06_milestone() {
    ...
    # Issue #1799: Use "swarm-memories/" path — consistent with write_swarm_memory() in helpers.sh.
    # The original path "swarms/" would never match any records since helpers.sh writes to "swarm-memories/".
    local swarm_files
    ...
    for sfile in $swarm_files; do
        ...
        # Issue #1799: write_swarm_memory() writes both memberCount and memberAgents array
        local members
        ...
        # Issue #1799: Check for emergent goals using goalOrigin field
        # write_swarm_memory() in helpers.sh now writes goalOrigin (fixed in this PR).
        # "agent-proposed" = planner proposed the swarm goal via governance/visionQueue
        # "emergent" = same as agent-proposed (alternate value)
        # "coordinator" = default, coordinator auto-spawned from GitHub issue routing
        local goal_origin
    ...
    if [ "$swarm_memory_count" -ge 1 ]; then
        ...
        criteria_report="${criteria_report}⏳ Criterion 4: Swarm memory — no dissolution records in swarm-memories/ yet"$'\n'
    fi
...
$(printf '%s' "$criteria_report" | sed 's/^/    /')
    ...
    The civilization has achieved Collective Action:
    - Spontaneous swarm formation (2+ swarms formed without god intervention)
    - Coalition size sufficient for parallel work (3+ agents in one swarm)
    - Emergent goals pursued (agent-proposed, not god-assigned)
    - Swarm memory persisted (institutional knowledge survives dissolution)

    Recommendation: Begin v0.7 planning. The civilization is ready for the next milestone.
MILESTONE_THOUGHT_EOF
...
All 4 success criteria from issue #1771 have been met:

$(printf '%s' "$criteria_report" | sed 's/\\n/\n/g')

### What This Means

The civilization has achieved collective action — agents form working groups spontaneously,
pursue emergent goals they proposed themselves, and persist institutional knowledge across
swarm lifetimes.

### Next Step

Begin v0.7 milestone planning.
...
while true; do
    ...
    # Every 20 iterations (~10 min): check v0.6 milestone completion (issue #1789, fixed by #1799)
    # Evaluates all 4 Collective Action success criteria from issue #1771.
    # Reads swarm dissolution records from s3://agentex-thoughts/swarm-memories/ (issue #1799 fix).
