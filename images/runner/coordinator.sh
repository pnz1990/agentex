...
ensure_state_fields_initialized() {
  ...
  if [ -z "$enacted" ]; then
    ...
    # Issue #1427: One-time migration — add enacted_topic_<topic> compat entries for any
    # existing old-format entries (format: "<topic>_<kv_pairs>") that lack a corresponding
    # enacted_topic_<topic> entry. This ensures that after PR #1420 merges (which changes
    # the coordinator to use enacted_topic_* format), previously-enacted topics like
    # release-v0, vision-feature, etc. are not re-enacted.
    # Extraction: parse topics from old-format entries like "release-v0_priority=critical approvals=..."
    local new_enacted_entries=""
    while IFS='|' read -r entry; do
      ...
      # Extract topic from entry: look for "topic_something" or "enacted_topic_topic" prefix
      # Entries look like: "2026-03-10T... topic_kv=val approvals=N ..."
      # or:                 "2026-03-10T... enacted_topic_topic approvals=N ..."
      # or:                 "<topic>_kv=val approvals=N ..." (no timestamp prefix)
      local entry_topic=""

      # Check if it's already new-format (enacted_topic_*)
      if echo "$entry" | grep -qE '(^| )enacted_topic_[a-zA-Z0-9_-]+'; then
        # Already new format — skip (no migration needed)
        continue
      fi

      # Try to extract topic from old-format: "TIMESTAMP topic_kv=val" or "topic_kv=val"
      # The topic is the part before the first underscore in the non-timestamp key token
      local key_token
      key_token=$(echo "$entry" | grep -oE '[a-zA-Z][a-zA-Z0-9_-]+_[^ ]+' | head -1)
      if [ -n "$key_token" ]; then
        # topic is everything before the first underscore-followed-by-non-alphanum
        # Handle topics with hyphens like "vision-feature", "circuit-breaker"
        # Key looks like "vision-feature_addIssue=..." or "circuit-breaker_circuitBreakerLimit=..."
        # Extract: take up to the first instance of "_[a-z,A-Z]" (indicating kv pair start)
        entry_topic=$(echo "$key_token" | grep -oE '^[a-zA-Z][a-zA-Z0-9-]*' | head -1)
      fi

      [ -z "$entry_topic" ] && continue

      # Check if enacted_topic_<entry_topic> already exists in enactedDecisions
      if echo "$enacted" | grep -qF "enacted_topic_${entry_topic}"; then
        continue  # Already has new-format entry for this topic
      fi

      # Also check in what we're about to add (avoid duplicates across old entries)
      if echo "$new_enacted_entries" | grep -qF "enacted_topic_${entry_topic}"; then
        continue
      fi

      # Add migration entry
      local migration_entry="$(date -u +%Y-%m-%dT%H:%M:%SZ) enacted_topic_${entry_topic} approvals=migrated"
      new_enacted_entries="${new_enacted_entries}${new_enacted_entries:+ | }${migration_entry}"
      [ "$silent" = "false" ] && echo "  Migrating enacted_topic_${entry_topic} (issue #1427)"
    done <<< "$(echo "$enacted" | tr '|' '\n')"

    if [ -n "$new_enacted_entries" ]; then
      local migrated_enacted="${enacted} | ${new_enacted_entries}"
      kubectl patch configmap "$STATE_CM" -n "$NAMESPACE" --type=merge \
        -p "{\"data\":{\"enactedDecisions\":\"$(echo "$migrated_enacted" | sed 's/"/\\"/g')\"}}" 2>/dev/null || true
...
tally_and_enact_votes() {
    ...
    while IFS= read -r topic; do
        ...
        local decision_key
        decision_key="$(echo "${topic}_${kv_pairs}" | tr '[:space:]' '_' | tr -s '_' | sed 's/_*$//')"

        # Issue #1427: check BOTH old-format keys (topic_kv=val) and new-format keys
        # (enacted_topic_topic) so that existing enactedDecisions entries (whether manually
        # added as enacted_topic_* compat entries or generated by an older coordinator) are
        # recognized correctly. This ensures re-enactment is blocked regardless of which
        # format was used to record the original enactment.
        if echo "$enacted" | grep -qF "$decision_key" || \
           echo "$enacted" | grep -qF "enacted_topic_${topic}"; then
