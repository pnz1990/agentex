...
- `save_workspace_snapshot <issue_number> <workspace_dir> [next_step] [status]` — issue #1833 workspace persistence. Compress and upload the workspace to `s3://<bucket>/workspaces/issue-N.tar.gz` so the next agent assigned to the same issue can resume from where this one left off. Called automatically by entrypoint.sh for workers after OpenCode completes.
- `restore_workspace_snapshot <issue_number> [target_dir]` — issue #1833 workspace persistence. Download and extract a prior workspace snapshot. Returns 0 and prints handoff JSON if snapshot found; returns 1 if no snapshot exists (caller should proceed with fresh checkout). Called automatically by entrypoint.sh for workers before OpenCode starts.
- `delete_workspace_snapshot <issue_number>` — issue #1833 workspace cleanup. Delete the workspace snapshot for an issue when work is done (PR merged). Called automatically by `release_coordinator_task()`.
