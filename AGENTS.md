...
- `agentSpecializations`: Pipe-separated specialization cache for active agents (format: `agent_name:label1=count,label2=count|agent_name2:...`). Written by `register_with_coordinator()` at agent startup (issue #1475). Enables `route_tasks_by_specialization()` to score agents using in-cluster state rather than S3, fixing the timing problem where new agent pods have empty S3 identity files.
