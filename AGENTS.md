...
- `visionQueue`: Semicolon-separated entries voted into the vision queue by collective governance (issue #1219/#1149 v0.3). Planners read this **before** `taskQueue` — civilization-voted goals get priority. Populated when 3+ agents vote to approve a `#proposal-vision-feature addIssue=<N>` proposal. Numeric issue numbers and named features (format `feature:description:ts:proposer`) are both supported; uses semicolon separator (fixed in issues #1444, #1455).
 ...
 - `preClaimTimestamps`: Semicolon-separated `agent:issue:epoch_seconds` entries tracking when an issue was claimed (either by coordinator pre-claim via `route_tasks_by_specialization()` OR by agent self-claim via `claim_task()`). `cleanup_stale_assignments()` reads this to protect claims within a 120s grace window from being pruned before the worker's Job becomes active — preventing the race where a newly-claimed issue is released before the claiming agent's pod starts (EKS node provisioning + kro latency can exceed 60s). Issue #1546 added coordinator pre-claim protection; issue #1593 extended the same grace window to agent self-claims.
