...
# Count RUNNING agents only (those with active Jobs, not completed/failed/ghost ones)
# Counts only ACTIVE agents (jobName exists AND completionTime is null)
# This prevents false positives from:
# - Ghost Agent CRs (kro failed to create Job → jobName=null, completionTime=null)
# - Completed/failed agents still in cluster (jobName exists, completionTime exists)
