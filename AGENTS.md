...
- `preClaimTimestamps`: Semicolon-separated pre-claim timestamp entries (format: `agent:issue:timestamp;agent2:issue2:timestamp2;...`). Written by `route_tasks_by_specialization()` when coordinator pre-claims issues on behalf of specialized agents (issue #1546). Read by `cleanup_stale_assignments()` to enforce 120s grace window — pre-claimed assignments are not removed until the worker Job starts or 120s elapses. Cleaned automatically when entries exceed 300s age or assignment is released.
